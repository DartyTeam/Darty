...
class MKMessage: NSObject, MessageType {
    ...
    init(message: LocalMessage) {
        ...
        switch message.type {
        case GlobalConstants.kTEXT:
            self.kind = MessageKind.text(message.message)
        case GlobalConstants.kPHOTO:
            let photoItem = PhotoMessage(path: message.pictureUrl)
            self.kind = MessageKind.photo(photoItem)
            self.photoItem = photoItem
        case GlobalConstants.kVIDEO:
            let videoItem = VideoMessage(url: nil)
            self.kind = MessageKind.video(videoItem)
            self.videoItem = videoItem
        case GlobalConstants.kLOCATION:
        ...
        case GlobalConstants.kAUDIO:
            let audioItem = AudioMessage(duration: 2.0)
            self.kind = MessageKind.audio(audioItem)
            self.audioItem = audioItem
        default:
            self.kind = MessageKind.text(message.message)
            print("ERROR_LOG Unknown message type")
        }
        
        self.senderInitials = message.senderInitials
        self.sentDate = message.date
        self.readDate = message.readDate
        self.incoming = AuthService.shared.currentUser?.id != mkSender.senderId
    }
}
