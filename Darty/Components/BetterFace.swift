...
//  Darty
//
//  Created by Руслан Садыков on 20.07.2021.
//
...
class BFImageView: UIImageView {
        ...
        set {
            ...
            if newValue != nil {
                if self.needsBetterFace {
                    self.faceDetect(aImage: newValue)
                }
            }
        }
    }
    ...
    func faceDetect(aImage: UIImage){
        print("asidojaiodjasid: ", aImage)
        let queue = DispatchQueue(label: "com.croath.betterface.queue", qos: .default, attributes: .concurrent)
        queue.async {
